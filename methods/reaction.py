...
class ReactionSpammer():
    async def reactionSpammer(token, chId, msgId, emoji):
        tokens = []
        ...
        async with ClientSession(headers=headers) as session:
            async with session.put(
                ...
                if r.status == 204:
                    print(Colors.white + "[" + Colors.green + "+" + Colors.white + f"] {tk} successfully reacted!")
                elif r.status == 404:
                    print(Colors.white + "[" + Colors.red + "x" + Colors.white + f"] {tk} couldn't react, {r.reason}!")
                elif r.status == 400:
                    print(Colors.white + "[" + Colors.red + "x" + Colors.white + f"] {tk} couldn't react, Unknown Emoji!")
                else:
                    text = await r.text()
